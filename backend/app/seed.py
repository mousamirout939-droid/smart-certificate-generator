...
def quiz_questions_for(course_code):
    ...
    return [
        ...
        {"id": "q5", "question": f"A learner wants to apply {title} at work. Which action is the strongest example of using the course knowledge?", "options": [f"Apply {example} without checking the outcome", f"Apply {example} and evaluate whether it solved the problem", "Use an unrelated process instead", "Avoid using the skill until someone else demonstrates it"], "correct_option": f"Apply {example} and evaluate whether it solved the problem"},
    ]
