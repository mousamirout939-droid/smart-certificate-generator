      ...
      <div className="space-y-5">
        {videos.map((video, index) => (
          <Card key={video.id} className="overflow-hidden">
            <div className="aspect-video bg-ink-900 relative">{video.youtube_id ? <><iframe className="w-full h-full" src={`https://www.youtube.com/embed/${video.youtube_id}?rel=0`} title={video.title} allow="accelerometer; autoplay; clipboard-write; encrypted-media; gyroscope; picture-in-picture; web-share" referrerPolicy="strict-origin-when-cross-origin" allowFullScreen /><a href={`https://www.youtube.com/watch?v=${video.youtube_id}`} target="_blank" rel="noreferrer" className="absolute inset-x-0 bottom-0 bg-black/70 text-white text-center text-sm py-2 hover:bg-black/85">Open this lesson on YouTube if the embedded player does not start</a></> : <div className="h-full flex items-center justify-center"><a href={`https://www.youtube.com/results?search_query=${encodeURIComponent(video.title)}`} target="_blank" rel="noreferrer" className="text-white underline">Find this lesson on YouTube</a></div>}</div>
